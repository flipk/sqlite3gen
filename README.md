...
The protobuf and xml features of this tool are optional.

If `xml` is specified, TinyXML2 is required, and `copy_to_xml`,
`copy_from_xml`, `export_xml`, and `import_xml` methods are added to
table classes, and `export_xml_all` and `import_xml_all` methods are
added to the `SQL_TABLE_ALL_TABLES` class.

if `protobuf` is specified, then `ENUM` data types are supported and
`PROTOID` tags may be applied to columns. It also requires libprotobuf.
...
This tool will emit a proto file suitable for feeding to google
...
The value of the `PACKAGE` line in the schema file will be used
for the `package` line in the proto file (and thus in the C++
namespace for the autogenerated messages).

## transations

There is a helper class for supporting SQL transactions.

```C++
SQL_TRANSACTION   xact(pdb, /*commit_on_delete*/ false);

xact.begin();

// multiple insert and update statements may go here
...
When `xact.begin()` is executed, a `BEGIN TRANSACTION` is performed.
All inserts and updates then begin going into a separate transaction
buffer. Once the operations have been deemed successful or not, the
transaction may be completed. If you simply allow the `xact` object
to be deleted, the `commit_on_delete` argument specifies what to do
by default (commit or rollback), or you may explicitely specify by
calling `xact.commit()` or `xact.rollback()`.
